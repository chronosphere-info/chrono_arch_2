	...
	function(dir, verbose=FALSE, attach=FALSE, rotate=FALSE){
		...
		for(i in 1:length(snapshots)){
			for(k in 1:length(vars)){
				ar2[i, k] <- paste0(vars[k] ,"_simulation=", names(snapshots)[i])
			}
		}
		...
		# the names
		dimnames(ar) <- list(age=snapshots, variable=vars)
